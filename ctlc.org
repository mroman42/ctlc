...
with the STLC will be made explicit in the [[*Propositions as types][next section]].
...
strategy will find the normal form if it
...
The use of monads for parsing was firstly discussed in cite:Wadler85,
...
example, it tests that any De Bruijn expression keeps is meaning when
...
Until this point, we have only used the function type. We are working
on the implicational fragment of the simply-typed lambda calculus we
described on the first [[*Typing rules for the simply typed \lambda-calculus][typing rules]].  We are now going to extend the
type system in the same sense we [[*Extending the simply typed \lambda-calculus][extended]] the simply-typed lambda
calculus. The following types are added to the type system
