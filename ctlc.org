...
been formalized in two Agda libraries whose complete code is presented
in the following links in navigable HTML format. Note that each
...
its expressive power, even if they are not part of usual presentations
...
All this machinery is used after constructing the higher-inductive type
