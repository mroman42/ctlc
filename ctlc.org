...
rebuild large parts of mathematics inside their structure using an
internal language.  Each topos is thus an universe of mathematics with
...
to get a different proof $\mathsf{p} = \mathsf{p}$ which we will call $\mathsf{loop}^{-1}$; moreover,
