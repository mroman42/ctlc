...
# Usar adyacencia para la composición -> No parece buena idea porque crea ambigüedad a veces
...
commute must follow $\varphi(0) = x$ and $\varphi(\mathrm{succ}(n)) = f(\varphi(n))$. Thus, in a certain sense, the 
...
\end{tikzcd}\]
This definition immediatly implies the equalities $0+m = \id(m) = m$
...
# perspectiva de etcs, estructural end vez de material.
