...
# TODO: Corregir apuntadores de los capítulos, los de página funcionan
...
# TODO: Sistemas de escritura en las formas normales, noetherianidad
...
in our usual set-theoretical foundations. In exchange, we will have
to deal with other problems such as non-terminating functions.
...
expressions, called /\lambda-terms/, and by some manipulation rules.
These terms can stand for functions or arguments indistinctly, and
...
precedence, i.e., $\lambda x. M N$ should be read as $\lambda x.(MN)$
...
\end{tikzcd}\]
This, however, should
be done avoiding the unintended bounding which happens
...
$\to_{\beta}$, which will be extended by transitivity to $\tto_{}_{\beta}$.

Ideally, we would like to define evaluation as a series of reductions
into a canonical form which could not be reduced
further. Unfortunately, as we will see later, it is not possible to
find, in general, that canonical form.
...
in the notion that any term which directly applies a function to the
...
\[
\Lambda(\skiabs(\lambda u. v u)) = \Lambda(v) = v.
\]
...
#+ATTR_LATEX: :options [Closed lambda terms and SKI combinators]
#+BEGIN_theorem
For any \lambda-term $M$,
\[
\Lambda(\skiabs(M)) =_{\beta\eta} M.
\]
#+END_theorem
#+BEGIN_proof
We can firstly prove by structural induction that $\Lambda([x].M) = \lambda x.\Lambda(M)$
for any $M$. In fact, we know that $\Lambda([x].x) = \lambda x.x$ for any 
variable $x$; we also know that
\[\begin{aligned}
\Lambda([x].MN) &= \Lambda(S([x].M)([x].N)) \\
          &= (\lambda x.\lambda y.\lambda z. xz(yz))(\lambda x.\Lambda(M))(\lambda x.\Lambda(N)) \\
          &= \lambda z.\Lambda(M)\Lambda(N);
\end{aligned}\]
also, if $x$ is free in $M$,
\[
\Lambda([x].M) = \Lambda(KM) = (\lambda x.\lambda y.x) \Lambda(M) =_{\beta} \lambda x.\Lambda(M);
\]
and finally, if $x$ is free in $U$,
\[
\Lambda([x].Ux) = \Lambda(U) =_{\eta} \lambda x.\Lambda(U)x\ .
\]
Now we can use this result to prove the main theorem. Again by
structural induction,

 * $\Lambda\skiabs(x) = x$;
 * $\Lambda\skiabs(MN) = \Lambda\skiabs(M)\Lambda\skiabs(N) = MN$;
 * $\Lambda\skiabs(\lambda x.M) = \Lambda([x].\skiabs(M)) =_{\beta\eta} \lambda x.\Lambda\skiabs(M) = \lambda x.M$.
#+END_proof
...
# chapter ?; and a detailed description on how to use the untyped \lambda-calculus
# as a programming language is given in ?.
...
\RightLabel{$(case)$}
\TIC{$\cterms{case c of [a].inr a; [b].inl b }\ctypes{:: B+A}$}
\RightLabel{$(abs)$}
\UIC{$\cterms{λc.case c of [a].inr a; [b].inl b }\ctypes{:: A+B \to B+A}$}

\AXC{$\cterms{z }\ctypes{:: A}$}
\RightLabel{$(inl)$}
\UIC{$\cterms{inl z }\ctypes{:: A+B}$}
\RightLabel{$(app)$}
\BIC{$\cterms{(λc.case c of [a].inr a; [b].inl b)(inl z) }\ctypes{:: B+A}$}
\RightLabel{$(abs)$}
\UIC{$\cterms{λa.(λc.case c of [a].inr a; [b].inl b) (λz.inl z) }\ctypes{:: A \to B + A}$}
\end{prooftree}
...
Note that Mikrokosmos implements a slightly more complex but nicer
to implement version of the $(case)$ rule that depends on functions instead of
abstractions. Namely,
\begin{prooftree}
\LeftLabel{$(case)$}
\AXC{$\Gamma \vdash p:A \to C$}
\AXC{$\Gamma \vdash q:B \to C$}
\AXC{$\Gamma \vdash m : A + B$}
\TIC{$\Gamma \vdash (\mathrm{case}\ m\ \mathrm{of}\ p;\ q) : C$}
\end{prooftree}                
...
# Properties of type algebra can be proved on Agda.
...
areas such as parsing, testing or system interaction; matching the
...
Haskell is
  ...
  * *lazy*, with /non-strict semantics/, meaning that it will not
    evaluate a term or the argument of a function until it is
    needed.

    In cite:hughes89, Hughes argued for the benefits of a lazy
    functional language, which could solve the efficiency problem.
...
the file =mikrokosmos.cabal=. It is used by the package managers
*stack* and *cabal* to provide the necessary libraries even if they
are not available system-wide. The *stack* tool is also used to package
the software, which is uploaded to Hackage.
