...
for all $\mu$ at the same time.  The notion of /natural transformation/ formalizes
this intuitive idea.
...
# The primary challenge that needed to be addressed was that the
# foundations of mathematics were unprepared for the requirements of
# the task. Formulating mathematical reasoning in a language precise
# enough for a computer to follow meant using a foundational system of
# mathematics not as a standard of consistency to establish a few
# fundamental theorems, but as a tool that can be employed in
# everyday mathematical work.
#  -- Vladimir Voevodsky. The origins and motivations of Univalent Foundations.
...
Topoi can be defined as locally cartesian closed categories with
 ...
 * A proof $\| \sum_{q : D} \mathrm{cut}_r(q) \|$ witnessing that the Dedekind cut is *inhabited*, that
   is, there exists some $q$ such that $r < q$.
...
*** Homotopy type theory III: Univalence
