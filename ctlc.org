...
- [ ] 5. Implementation of Lambda-expressions
- [ ] 6. User interaction
- [ ] 7. Usage
...
- [X] 11. Categories
- [X] 12. Functors and natural transformations
- [X] 13. Constructions on categories
- [ ] 14. Universality and limits
- [ ] 15. Adjoints, monads and algebras
- [ ] 16. Presheaves
- [ ] 17. Cartesian closed categories and lambda calculus
- [ ] 18. Working in cartesian closed categories
- [ ] 19. Locally cartesian closed categories and dependent types
- [ ] 20. Working in locally cartesian closed categories
- [ ] 21. Topoi
- [ ] 22. Martin-Löf type theory
- [ ] 23. Homotopy type theory

*** TODO [#A] Terminar Presheaves
...
\lambda-calculus and simply typed \lambda-calculus, and we study their
...
and a function is completely determined by what its outputs are.
This vision is called */extensional/* (see cite:selinger13).
...
In constrast, untyped \lambda-calculus presents some problems such as
...
$\lambda x.\ x^2 + x$; and $p(2)$ is written as $(\lambda x.\ x^2+x)(2)$. In general,
...
written as $M[N/x]$ and defined inductively as
\[\begin{aligned}
x[N/x] &\equiv N,\\
...
/well-defined/, but only /well-defined up to a change on the name of the variables/.
...
The three properties can be represented respectively (left: confluence, center:
quasidiamond, right: diamond) as
...
for any variable $\red{x}$ and any terms $\red{N},\red{N'},\red{M},\red{M'}$ such that $\red{M}\rhd\red{M'}$
and $\red{N}\rhd\red{N'}$:
...
In order to prove that this relation has the diamond property, we will
define a reduction of a term that can be itself reached from any of its
parallel one-step reductions. We begin proving a lemma on 
...
\[
\mathtt{SKI} ::= x \mid (\mathtt{SKI}\ \mathtt{SKI}) \mid S \mid K \mid I,
\]
...
the SKI-term $U$ on the variable $x$ is written as $[x].U$ and defined
recursively as
...
is in fact Turing complete (see [[sec-fixed-points]]).
...
#+LATEX: \ctparttext{\color{black}\begin{center}
We have developed *Mikrokosmos*, an untyped and simply typed
\lambda-calculus interpreter written in the purely functional
...
statements of intuitionistic propositional logic inside it.
#+LATEX: \end{center}}
...
categorical monads, see the chapter on Category Theory.
...
They can be accessed directly from the following direction.
...
It is a particular instance of a deeper fact we will detail on the chapter
on Categorical Logic: exponentials are defined by the adjunction
...
would be encoded as $2 = \lambda s.\ \lambda z.\ s (s (z))$.
...
$\mathtt{true}(a,b) = a$, and $\mathtt{false}(a,b) = b$.
...
#+LATEX: \ctparttext{\color{black}\begin{center}
Categories will be the framework we will use to study the fundamental
notion of function composition inside mathematics. They provide a
useful language to talk about theories, mathematic and logic. In
particular, adjoint functors will be one of the key insights we will
use to understand and expand the connection between computation and
logic we have described so far.  This section is based on cite:maclane78.
#+LATEX: \end{center}}
...
sets or proper classes.  We want to talk about categories containing
subsets of the class of all sets, and thus it is necessary to allow
the objects to form a proper class (which is not itself a set) in
order to avoid inconsistent results such as the Russell's paradox.
This is why we will consider a particular class of categories of small
 ...
 * a collection (sometimes denoted $\mathrm{obj}({\cal C})$ or simply ${\cal C}$), whose
   elements are called *objects*; and
 * a collection, whose elements are called *morphisms*.

Every morphism $f$ is assigned two objects: a *domain*
and a *codomain*; a common notation to indicate that $f$
has domain $A$ and codomain $B$ is to write $f \colon A \to B$.
...
$f,g,h$; and it must have an /identity/ morphism $\id_{A}\colon A \to A$ for each
...
As we promised, /to be isomorphic to/ is an equivalence
...
is itself an isomorphism, $(f \circ g)^{-1} = g^{-1} \circ f^{-1}$.
...
and the pullback object is usually called a /fibered product/.
...
axioms hold. A model $H \colon \mathbb{G} \to {\cal C}$ is determined by an object of ${\cal C}$
and some /morphisms of the catgeory/ with the above signature
...
#+END_corollary
#+BEGIN_proof 
It preserves finite products because it preserves all limits, and thus,
...
$d : A \to B^A$, then each $f : B \to B$ has a fixed point $b : B$ such
 ...
 * *Russell's paradox* follows because $\in \colon \mathrm{Sets} \to 2^{\mathrm{Sets}}$ would be point-surjective
 ...
 * *Gödel first incompleteness theorem* and *Tarski's theorem* follow a similar
   ...
   *consistent* if there exists a morphism $\mathrm{not} \colon 2 \to 2$ such that $\mathrm{not}\circ \varphi \neq \varphi$ for
   every $\varphi : A \to 2$. We say that *satisfiability* is definable if there exists a 
   map $\mathrm{sat} : A \times A \to 2$ such that for every /predicate/ $\varphi \colon A \to 2$, there exists
   a /Gödel number/ $c : A$ such that $\mathrm{sat}(c,a) = \varphi(a)$. The fixed point theorem says
   that, if satisfiability is definible in a theory, it must be inconsistent.
...
taking $C = \mathbb{N} \times D$ when applying the induction principle in
\[
...
endofunctors (as seen in Examples [[example-naturalnumbersobj]] and[[
example-listalgebra]]). Binary trees, as described in Example [[example-bintree]],
are the initial algebra for $(1 + \mathbb{N} \times T \times T)$.
#+END_exampleth
...
Excluded Middle, we do not assume that $P \vee \neg P$ for any arbitrary
proposition $P$. Note that we do not negate it, neither; we are
/agnostic/ regarding it.
...
in our theory, we expect it to be a numeral of the form $\mathrm{succ}(\mathrm{succ}(\dots \mathrm{zero} \dots))$
for an some number of $\mathrm{succ}$ applications. Inductive definitions
can only compute with natural numbers when they are on this form.  However,
if we were to postulate the Law of Excluded Middle, $\mathrm{LEM}\colon P \vee \neg P$, we should accept
the existence of numbers such as
\[ \mathrm{if}\ \mathrm{LEM}(\mathrm{RiemmanHypothesis})\ \mathrm{then}\ 0\ \mathrm{else}\ 1,\]
...
In any case, classical mathematics is a particular case of constructive
...
#+ATTR_LATEX: :options [Diaconescu's theorem]
#+BEGIN_theorem
...
$f(V)$ are equal to $0$ or not (by induction). If $f(U) = 1$ or $f(V) = 0$, we
...
generated, we can apply symmetry to get a different proof $\mathsf{p} = \mathsf{p}$ which we
...
Cordón, David Charte, Marta Andrés, José Carlos Entrena, Pablo Baeyens,
Antonio Checa, Daniel Pozo, and Sofía Almeida in particular, for testing the
