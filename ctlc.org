...
logical system by Church suffered from the */Rosser-Kleene paradox/*
and types were a method to avoid it, as
...
a term of type $A \to B$; because it would expect only a term of type $A$.
*Typing rules* will give terms their desired expressive power. Only a subset
...
$A_1\dots,A_n$ and $B_1,\dots,B_n$ if $\overline{\psi} A_i = \overline{\psi} B_i$ for any $i = 1,\dots,n$. We say that it
...
we show equivalent to a category containing lambda theories in
Theorem [[th-equivalence-ccctl]].
#+END_exampleth
...
we know that if satisfiability is definable in a theory then
