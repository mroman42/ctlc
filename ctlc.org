...
# TODO: Show the proof with Gentzen deduction trees!
...
# TODO: Hom functors as an example of covariant and contravariant functors
*** Product categories
...
#+ATTR_LATEX: :options [Conditions of existence bifunctors]
#+BEGIN_proposition
...
a functor $-^{-} \colon \mathtt{Cat}^{op} \times \mathtt{Cat} \to \mathtt{Cat}$ sending any two categories
...
**** Limits                                                       :ignore:
#+ATTR_LATEX: :options [Diagonal functor]
#+BEGIN_definition
The *diagonal functor* $\Delta \colon {\cal C} \to {\cal C}^{{\cal J}}$ sends each object $c \in {\cal C}$ to a
...
The diagonal functor sends a morphism $g \colon c \to c'$ to a natural
...
&\phantom{gap}&& &&& & \phantom{gap} &&\\
  & \cdot \ar{ld}\ar{r}\ar{drr} & \cdot \ar{dr}  & \ar[Rightarrow, shorten <= 3mm, shorten >= 14mm]{uurrrr}[xshift=-3mm]{\Delta}  &&&   & \cdot \ar{ld}\ar{r}\ar{drr} & \cdot \ar{dr} & \\
\cdot \ar{rrr} &&& \cdot \ar[Rightarrow, yshift=3mm,swap, shorten <= 2mm, shorten >= 2mm]{rrr}{F}           &\phantom{gap}&& \cdot \ar{rrr} &   &   & \cdot \\
&{\cal J}&& &&& &F{\cal J}&&
\end{tikzcd}\]
...
a way to recover all the information that was encoded in $c$ by the
diagonal functor. As we did with products, we want to find the cone
that best encodes that information; a universal cone, such that every
other cone factorizes through it. Diagramatically, an $r$ such that,
for each $d$,
\[\begin{tikzcd}[column sep=tiny, row sep=tiny]
&d 
...
limit by a universal natural transformation between the two diagonal
functors.
...
\[\begin{tikzcd}
d\dar[dashed,swap]{\exists! f} & \Delta d \dar[swap,dashed]{Sf}\drar{g} & \\
r & \Delta r \rar[swap]{v} & c
\end{tikzcd}\]
commutes.
...
** Kan extensions
*** TODO Profunctors
# Where could I write the definition of profunctors?
# Maybe as part of an example.
