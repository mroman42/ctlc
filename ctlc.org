...
** To-do list                                                     :noexport:
*** TODO Teorema de Hedberg y cómo recuperar conjuntos con ETCS
*** TODO Añadir paradoja de Escardó para motivar topología y computación
*** TODO Añadir ejemplo topológico de nudo que no se cierra
 ...
 * inside the Johnstone's *topological topos*, we can reason with topological
   ...
   certain hypothesis, it is safe to assume that all the functions we can
...
of $p = p$ which we will call $\mathsf{loop}^n$; these are the elements
