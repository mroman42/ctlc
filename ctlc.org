...
#+ATTR_LATEX: :options [Lawvere algebraic theory]
#+BEGIN_definition
An *algebraic theory* cite:bauer17 is a category $\mathbb{A}$ with all finite products whose objects
forming a sequence $A^0,A^1,A^2,\dots$ such that $A^m \times A^n = A^{m+n}$ for
any $m,n$.
#+END_definition
# From this definition, it follows that $A^0$ must be the terminal object.
...
formulations fo the same theory, with different but equivalent axioms. For
...
The identity is the morphism $(x_1\dots x_n \vdash \pair{x_1,\dots, x_n})$. The kth-projection from $A^n$ is the
...
The category of models $\Mod_{\Set}(\mathbb{A})$ is the usual category of groups; note that the natural
...
#+ATTR_LATEX: :options [Preseaf categories are cartesian closed]
#+BEGIN_exampleth
...
# TODO: isomorphism of types. monos and epis of types?
...
$\mathrm{ifelse} : \mathtt{bool} \to C \to C \to C$ and $\mathrm{rec} : C\to (\mathtt{nat} \to C \to C) \to \mathtt{nat}\to C$; and the
axioms
...
We consider the category $\lambda\mhyphen\mathtt{Thr}$ of \lambda-theories with translations,
note that the identity and composition of translations are translations.

**** TODO Sintactic category                                      :ignore:
# Models are functors from the sintactic category

**** TODO Internal language                                       :ignore:
*** Internal languages
...
of equivalence relations, \beta-reductions and \eta-reductions.
 ...
 2) Equality is reflexive, transitive and symmetric; that is, it is an
 ...
 4) Reduction rules for products define the following equalities
    for pair constructors
    \begin{prooftree}
 ...
 5) Reduction rules for functions translate into the following equations,
    where the second one is directly derived from the \beta-rule
    \begin{prooftree}
    ...
    \BIC{$$}
    \end{prooftree}
    and the following ones, where the second one is directly derived from
    the \eta-rule
    \begin{prooftree}
