...
** To-do list                                                     :noexport:
*** TODO Añadir lista de referencias principales
  - Selinger
  - MacLane, Categories for the working mathematician
  - Homotopy type theory book
  - nLab

*** TODO Repasar todo sustituyendo definiciones de entorno por negrita
*** TODO Quote Lawvere
Buscar en el libro de Conceptual Mathematics
*** TODO Colorear reglas del cálculo lambda simplemente tipado
Especialmente en la parte de deducción natural, evitarlo en otros
puntos o usarlo consistentemente.

*** TODO Considerar sólo un caso de límites con su dual, en lugar de dos.
*** TODO Añadir interpretación de Prop como subobjeto clasificador.
*** TODO Reescribir parte de teoría de tipos, añadir univalencia solo al final.
*** TODO Citas en cada capítulo
*** TODO Agda exportado a HTML.
...
subject has been made possible by Prof. Pedro García-Sánchez and
Prof. Manuel Bullejos.  They have provided me with corrections and
...
Cordón, David Charte, Marta Andrés, Pablo Baeyens, Antonio Checa,
Daniel Pozo, and Sofía Almeida in particular, for testing the
interpreter and providing useful discussion and questions on the
topics of this text. Prof. Luis Merino, Prof. Juan Julián Merelo and
Braulio Valdivielso have made possible and participated on the
organization of meetings and workshops on these ideas.
...
follows the =classicthesis= [[http://www.latextemplates.com/templates/theses/2/thesis_2.pdf][template]] by *André Miede* and uses modifications
by Adrián Ranea, Alejandro García and David Charte. The =minted=
package has been used for code listings and the =tikzcd= package has
been used for commutative diagrams. The document is released under a Creative Commons
BY-SA 3.0 license, while the source code can be redistributed under
...
In constrast, untyped \lambda-calculus presents some problems such as non-terminating
functions.
...
Equivalently, they are given by the following Backus-Naur form,
\[
\mathtt{Term} ::= x \mid (\mathtt{Term}\ \mathtt{Term}) \mid (\lambda x.\mathtt{Term}),
\]
...
when evaluating. For instance, in the previous example, we
...
This equivalence relation will be defined formally on the next section.
In practice, it is common to follow the /Barendregt's variable convention/,
which simply assumes that bound variables have been renamed to be distinct.
...
argument it takes can be reduced to that function. That is, $\lambda x.M x$
can be reduced to $M$.

#+attr_latex: :options [\eta-reduction]
#+begin_definition
The *\eta-reduction* is the smallest relation on \lambda-terms satisfiying 
...
*** Confluence
It is not possible in general to evaluate a \lambda-term
...
transitive closure $\tto$, for any $M,N,P \in {\cal S}$,  $M \tto N$ and $M \tto P$
imply the existence of some $Z \in {\cal S}$ such that $N \tto Z$ and $P \tto Z$.
#+end_definition

Given any binary relation $\to$ of which $\tto$ is its reflexive transitive
closure, we can consider three apparently related properties:

  * the *confluence property* (also called /Church-Rosser property/) we have just defined;
  * the *quasidiamond property*, which assumes $M \to N$ and $M \to P$, and
  * the *diamond property*, which is defined substituting $\tto$ by $\to$ on
    the definition on confluence.

Diagrammatically, the three properties can be represented
respectively as
...
the quasidiamond does not. In order to prove confluence for a given relation, we
must use the diamond property instead of the quasidiamond property.
...
as the smallest relation satisfying that the following properties
  ...
  * reflexivity for variables, $\red{x} \rhd \red{x}$;
  * parallel application, $\red{MN} \rhd \red{M'N'}$;
  * \lambda-abstraction congruence, $\red{\lambda x.N} \rhd \red{\lambda x.N'}$;
  * parallel substitution, $\red{(\lambda x.M)N} \rhd \red{M'[N'/x]}$;
  * and extensionality, $\red{\lambda x.M x} \rhd \red{M'}$, if $\red{x} \not\in \mathrm{FV}(\red{M})$,

hold for any variable $\red{x}$ and any terms $\red{N},\red{N'},\red{M},\red{M'}$ such that $\red{M}\rhd\red{M'}$ and $\red{N}\rhd\red{N'}$.
...
any \lambda-terms $\red{P},\red{P'}$,

  1) if $\red{P} \to_{\beta\eta} \red{P'}$, then $\red{P} \rhd \red{P'}$.
  2) if $\red{P} \rhd \red{P'}$, then $\red{P} \tto_{\beta\eta} \red{P'}$;
#+end_lemma
#+begin_proof
1) All possible ways in which we can arrive at $P \to_{\beta\eta} P'$ imply $P \rhd P'$.
   They are
     ...
     * $\lambda x. Mx \to M$, where $x \not\in \mathrm{FV}(M)$ and we can apply extensionality for $\rhd$
       and reflexivity.

2) We can prove this by structural induction on the derivations of $M \rhd M'$. The
   possible ways in which we arrive at it are
...
#+end_lemma
#+begin_proof
By structural induction on the derivations of $M \rhd M'$, depending
on what was the last derivation rule we used, we have the following
cases.

  * Reflexivity, $M = x$. If $x=y$, we simply use a$U \rhd U'$; if $x \neq y$,
...
we need
  ...
  * and to install the Mikrokosmos Jupyter kernel as usual.
...
#+LATEX: \ctparttext{\color{black}\begin{center}
This section is based on cite:maclane78.
#+LATEX: \end{center}}
...
isomorphism. Diagramatically, we compose
#+latex: \\[-20pt]
\begin{prooftree}
\AXC{\begin{tikzcd}[fragile,ampersand replacement=\&]
F'FX \rar{f}\& Y
...
\]
Note how every homomorphism from the free monoid is determined by how
it acts on the generator set.

This notion of forgetful and free functors can be generalized to
...
term $(x_1\dots x_n \vdash x_k)$, and it is easy to check that these projections make $A^n$ the n-fold
product of $A$.
...
$r : \mathtt{D}\to (\mathtt{D} \to \mathtt{D})$ and $s : (\mathtt{D}\to \mathtt{D})\to \mathtt{D}$ such that $r\ (s\ f) \equiv f$ for each
$f : \mathtt{D}\to \mathtt{D}$ and $s\ (r\ x) \equiv x$ for each $x : \mathtt{D}$. We assume that each term of the
 ...
 * the *existence of fixed points* for any term of *in untyped \lambda-calculus* 
   given as a theory like in Example [[example-untypedlambda-theory]] follows from 
   the fact that there exists a type isomorphism (in particular, a point-surjection)
   $\mathtt{D} \to (\mathtt{D}\to \mathtt{D})$;
...
equivalent the existence a type with an elimination rule representing
...
language with a type $\mathbb{N}$ and an elimination principle $\mathrm{ind}$ given by the
...
\\
Although this induction principle is weaker than the recursion
principle System T offered, $\mathrm{rec} : D\to (\mathbb{N} \to D \to D) \to \mathbb{N} \to D$,
it can be recovered using the cartesian closed structure and
taking $C = \mathbb{N} \times D$ in when applying the induction principle in
\[
\mathrm{rec}\ c_0\ c_s\ n \equiv
\pi_1\ (\mathrm{ind}\ \pair{c_0,0}\ (\lambda m.\ \pair{c_s\ m\ d, \mathrm{succ}\ m})\ n).
\]
It can be checked that $\mathrm{rec}$ is such that $\mathrm{rec}\ c_0\ c_s\ 0 \equiv c_0$, and
$\mathrm{rec}\ c_0\ c_s\ (S\ n) \equiv c_s\ n\ (\mathrm{rec}\ c_0\ c_s\ n)$.

Finally, note that $\mathrm{ind}$ can be directly recovered from $\mathrm{rec}$, and the
...
following form for some fixed objects $C_0,\dots,C_n$;
\[
P(X) = C_0 + C_1 \times X + C_2 \times X^2 + \dots + C_n \times X^n.
\]
Where the initial algebra can be seen to correspond to an inductive
...
#+BEGIN_exampleth
Let $T$ be a type generated by the constructors $\mathrm{nil} : T$ and
$\mathrm{node} : \mathbb{N} \to T \to T \to T$. It can be seen as the data structure
of a binary tree of naturals, where "$\mathrm{nil}$" is an empty leaf and "$\mathrm{node}$"
...
\mathrm{node}\ 5\ 
(\mathrm{node}\ 3\ 
(\mathrm{node}\ 1\ \mathrm{nil}\ \mathrm{nil})\ 
(\mathrm{node}\ 2\ \mathrm{nil}\ \mathrm{nil}))\ 
(\mathrm{node}\ 4\ \mathrm{nil}\ (\mathrm{node}\ 6\ \mathrm{nil}\ \mathrm{nil})).
\]
#+END_exampleth

**** Initial algebras in presheaves                               :ignore:
...
form $\varphi = yh$ for a unique $h \colon C \to C'$. Note how ${\cal D}$ can be fully and
faithfully embedded inside $\Set^{{\cal C}^{op}}/A$ with a functor $I \colon {\cal D} \to \Set^{{\cal C}^{op}}/A$.
...
It remains to show that this functor in fact determines an equivalence.
...
$\|A\|$ by postulating that two of its proofs $p,q : \|A\|$ should be equal,
...
As an example, there are four distinct elements of
\[
\sum_{(n,m) : \mathbb{N} \times \mathbb{N}} (n + m = 3),
\]
each one of them providing an ordered pair of naturals that add up to $3$.
...
How can we build truncations inside our theory? It can be proved (see cite:awodey04)
...
* Type theory and topoi (abstract)                                   :ignore:
#+LATEX: \ctparttext{\color{black}\begin{center}
...
* Type theory and topoi
** Universes of mathematics
*** Topoi
A *topos* (plural /topoi/) is a locally cartesian closed category with
a subobject classifier where every finite limit exists. Usually,
*W-topoi*, or topoi with a natural numbers object (in the sense of
Example [[example-naturalnumbersobj]]), are considered cite:leinster10.

Each topos is a model of constructive mathematics; and we can reason
in its internal logic and rebuild mathematics internally in its
language. Each topos is thus an universe of mathematics with different
axioms and interpretations cite:bauer2017five; for example,

 * inside the Hayland's *realizability topos*, every function is computable
   and we can study Kleene's realizability theory cite:vanoosten08;
 ...
 * the *Dubuc topos* provides a non paradoxical formalization of notion of 
   ...
   differential geometry inside it cite:dubuc89;

 * inside the Johnstone's *topological topos*, we can reason with topological
   spaces and continuous functions between them cite:johnstone79, and under
   certain assumptions, it is safe to assume that all the functions we can
   build are automatically continuous cite:escardo15.
...
models in categories like these.
...
studying corresponds to *Martin-Löf type theory* cite:nordstrom90.

*** Programming inside Martin-Löf type theory
...
programming language with a powerful enough type system; examples
of these include
 ...
 * *Idris*, a programming language, which implements a different
   version of identity types than that in standard type theory;
 ...
 * *NuPRL* cite:nuprl86 a proof assistant implementing Martin-Löf
   ...
   one in how it treats equality types;
...
*dyadic* positive rationals $D$; that is, over the rationals of the form
$a / 2^b : D$ for some $a,b : \mathbb{N}$. We have also written a library of lemmas and
theorems about the natural numbers and the dyadic numbers necessary to
prove that our constructions are well-defined.

Natural numbers are constructed as initial algebras and dyadic rationals
are constructed as pairs of naturals with a normalization property that
ensures unique representation of each rational. A fraction $a/2^b$ is
/normalized/ if $a$ is an odd number or if $b$ is exactly zero.
\[
D = \sum_{(a,b) : \mathbb{N} \times \mathbb{N}} \left\|\mathrm{odd}(a) + \mathrm{isZero}(b) \right\|
\]

A real number $r : \mathbb{R}^+$ is thus interpreted as

 * a *Dedekind cut* over the positive dyadic rationals, $\mathrm{cut}_r : D \to \Omega$,
   where $\mathrm{cut}(q)$ is true for some $q : D$ when $r < q$;

 * some proof witnessing that it is *inhabited*, $\| \sum_{q : D} \mathrm{cut}_r(q) \|$;

 * and functions that witness that the cut is /round/ (open lower-bounded interval);
   that is, for each $q : D$ in the cut, there is a smaller $p : D$ in the cut;
   and, if some $p,q : D$ we have $p < q$ and $p$ in the cut, $q$ must also be in the cut;
   symbolically
   \small
   \[
...
Streicher's Axiom K cite:streicher93, which is independent from
Martin-Löf type theory.
...
transitivity, could be modeled into a groupoid cite:hofmann98.
If we also consider equalities between proofs of equality,
and equalities between proof of equalities between equalities
...
paths, equalities between equalities are homotopies between paths
and so on (some work on this hypothesis can be read in cite:tamsamani96).

In any case, as the existence of these non-trivial structure is
independent of the theory, we need to introduce a new axiom or types
with nontrivial equalities if we want to profit from this interpretation. 
The introduction of Voevodsky's
Univalence Axiom will lead to Homotopy Type theory, an extension of
...
$\mathsf{loop} : \mathsf{p} = \mathsf{p}$. Because it is freely generated, we can apply symmetry
to get a different proof $p = p$ which we will call $\mathsf{loop}^{-1}$; moreover,
we can transitivity $n$ times to $\mathsf{loop}$ for an arbitrary $n$, to get a new proof
of $p = p$ which we will call $\mathsf{loop}^n$, these loops are the elements
of $\pi_1(\mathbb{S}^1, \mathsf{p})$.

In this setting, results such as the Van Kampen theorem
or the construction of Eilenberg-MacLane spaces have been formalized
(see cite:hottbook).
...
\]
\normalsize
It can be shown that, for any pair of types $A,B : {\cal U}$, there exists
...
#+BEGIN_definition
For any pair of types $A,B : {\cal U}$, $\mathrm{idtoeqv} : (A =_U B) \to (A \simeq B)$ is
...
as a proof, inside this synthetic homotopical setting, of the fact
that the foundamental group of the circle is $\mathbb{Z}$.
