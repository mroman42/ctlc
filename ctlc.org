...
**** Presheaves are locally cartesian closed                      :ignore:
*** Dependent types
...
type, $\Gamma \vdash a : A$ is represented by a morphism from the terminal object
of $\Gamma/A$, that is,
\[\begin{tikzcd}
...
$\pi_B \colon \intr{\Gamma, a:A, b:B} \to \intr{\Gamma, a:A}$, the type $\sum_{a:A}B$ is given by the
object
\[
