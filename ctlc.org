...
where $1$ is a one-element type and $\iota$ would be any /basic type/.
#+end_definition
...
can be found using the following recursive definition of $\mathtt{unify}(A_1,\dots,A_n;B_1,\dots,B_n)$.
  ...
  1) $\mathtt{unify}(\sigma;\sigma) = \mathrm{id}$ and $\mathtt{unify}(\iota,\iota) = \mathrm{id}$;
  2) $\mathtt{unify}(\sigma;B) = (\sigma \mapsto B)$, the substitution that only changes $\sigma$ by $B$;
     if $\sigma$ does not occur in $B$. The algorithm fails if $\sigma$ occurs in $B$;
  3) $\mathtt{unify}(A;\sigma)$ is defined symmetrically;
  4) $\mathtt{unify}(A_1 \to A_2, B_1\to B_2) = \mathtt{unify}(A_1,A_2;B_1,B_2)$;
...
Where $\sigma$ is any type variable. The two types have no unifier if and only if $\mathtt{unify}(A,B)$ fails.
#+end_lemma
#+begin_proof
It is easy to notice that, by induction, if $\mathtt{unify}(A;B)$ exists, it is in fact an unifier.

We now prove that, if $A$ and $B$ can be unified, $\mathtt{unify}(A,B)$ is the most general unifier.
For instance, in the clause 2, if we call $\psi = (\sigma \mapsto B)$ and $\eta$ where another unifier,
then $\eta\sigma = \overline{\eta}\sigma = \overline{\eta} B = \overline{\eta}(\psi(\sigma))$; hence $\overline{\eta} \circ \psi = \eta$. A similar argument can be applied
to clauses 3 and 4. In the clause 5, we suppose the existence of some unifier $\psi'$. The recursive
call gives us the most general unifier $\rho$ of $A_1,\dots,A_n$ and $B_1,\dots,B_{n}$; and since it is more
general than $\psi'$, there exists a $\varphi$ such that $\overline{\varphi} \circ \rho = \psi'$. Now, $\overline{\varphi}(\overline{\rho}A) = \psi'(A) = \psi'(B) = \overline{\varphi}(\overline{\rho} B)$,
hence $\varphi$ is a unifier of $\overline{\rho}A$ and $\overline{\rho}B$; we can take the most general unifier to be $\psi$, so
$\overline{\varphi}' \circ \psi = \overline{\varphi}$; and finally, $\overline{\varphi}' \circ (\overline{\psi} \circ \rho) = \overline{\varphi} \circ \rho = \psi'$.
...
#+begin_theorem
<<thm-typeinfer>>
The algorithm $\mathtt{typeinfer}(M,B)$, defined as follows, finds the most general substitution
...
and fails otherwise.
  ...
  2) $\mathtt{typeinfer}(\Gamma \vdash MN : B) = \overline{\varphi} \circ \psi$, where $\psi = \mathtt{typeinfer}(\Gamma \vdash M : \sigma \to B)$ and
     $\varphi = \mathtt{typeinfer}(\overline{\psi}\Gamma \vdash N : \overline{\psi}\sigma)$ for a fresh type variable $\sigma$.
  3) $\mathtt{typeinfer}(\Gamma \vdash \lambda x.M : B) = \overline{\varphi} \circ \psi$ where $\psi = \mathtt{unify}(B; \sigma' \to \sigma)$ and
     $\varphi = \mathtt{typeinfer}(\overline{\psi}\Gamma, x:\overline{\psi}\sigma' \vdash M : \overline{\psi}\sigma)$ for fresh type variables $\sigma,\sigma'$.
...
#+end_theorem
#+begin_proof
The algorithm terminates by induction on the size of $M$. On the first case, the
type is obviously the most general substitution by virtue of the previous Lemma [[lemma-unification]].
On the second case, the typing rules require $M$ and $N$ to be of type $\sigma \to B$
and $\sigma$ respectively; by induction, the given types are the most general possible ones;
that is, given any other substitution $\rho$, $\overline{\rho}M$ should be of the form $\sigma \to B$ for some
type variables, and therefore, $\overline{\rho} = \overlrine{\rho}' \circ \psi$.
...
*** TODO Extending the simply typed \lambda-calculus
