 ...
 * ${\cal C}_0$, a collection[fn:collection] whose elements are called *objects*[fn:objectnotation], and
...
between them. It is written as $\mathrm{hom}(A,B)$. The set of *endomorphisms*
of an object $A$ is the hom-set $\mathrm{end}(A) = \mathrm{hom}(A,A)$.
#+end_definition

Sometimes, when considering a hom-set, it will be useful to explicitly specify
the category on which we are working as $\mathrm{hom}_{{\cal C}}(A,B)$.
...
[fn:objectnotation]: We will sometimes write this class of objects
of a category ${\cal C}$ as $\mathrm{obj}({\cal C})$, but it is common to use
simply ${\cal C}$ to denote it.
...
morphism has a left-side inverse and a right-side inverse, they are an
inverse, and they are equal.
#+end_proposition
#+begin_proof
Given $f : A \to B$ with inverses $g_1,g_2 : B \to A$; we have that
\[
g_1 = g_1 \circ \mathrm{id}_A = g_1 \circ (f \circ g_2) = 
(g_1 \circ f) \circ g_2 =
\mathrm{id} \circ g_2 = g_2.
\]
...
#+begin_exampleth
The category $\mathtt{Set}$ is defined as the category with all the
...
#+begin_definition
A *functor* is a morphism of categories. Given two categories ${\cal C}$ and ${\cal D}$,
a functor between them $F : {\cal C} \to {\cal D}$ is given by
...
such that
  ...
  * $F(\mathrm{id}_A) = \mathrm{id}_{FA}$, identities are preserved;
...
It is trivial to notice that the composition of faithful (respectively,
...
The identity morphism of any object $\pair{c,d}$ is $\pair{\mathrm{id}_c, \mathrm{id}_d}$, and composition is
...
Given a bijection $\varphi$, we define $u = \varphi(\mathrm{id}_r)$.
...
sending the natural transformation $\alpha \colon \mathrm{hom}_{D}(r,-) \xrightarrow{.} K$
...
$Y \colon D^{op} \to \mathtt{Set}^{D}$, is defined with the arrow function
...
An adjunction determines a *unit* and a *counit*;
 ...
 1) the *unit* is natural transformation made with universal arrows $\eta\colon I \xrightarrow{.} GF$, where
 ...
 2) the *counit* is natural transformation made with universal arrows $\varepsilon \colon FG \xrightarrow{.} I$, where
...
operations $\wedge,\vee$ satisfying
...
\[
x = x \wedge (a \vee y) = (x \wedge a) \vee (x \wedge y) =
(y \wedge a) \vee (x \wedge y) = y \vee (x \wedge a) = y.
\]
#+end_proof
...
\[
z \wedge x \leq
(\overline{x} \vee y) \wedge x \leq
(\overline{x} \wedge x) \vee (y \wedge x) \leq
y \wedge x \leq
y;
\]
...
z = 
z \wedge 1 =
z \wedge (\overline{x} \vee x) =
(z \wedge \overline{x}) \vee (z \wedge x) \leq 
(z \wedge \overline{x}) \vee y \leq
z \vee y.
\]
#+end_proof
...
In any Heyting algebra,
  ...
  4) $\neg\neg (x \wedge y) = \neg \neg x \wedge \neg \neg y$,
  5) $(x \impl x) = 1$,
  6) $x \wedge (x \impl y) = x \wedge y$,
  7) $y \wedge (x \impl y) = y$,
  8) $x \impl (y \wedge z) = (x \impl y) \wedge (x \impl z)$.
...
We can prove the inequalities using the definition of implication.
  ...
  4) We know that $x \wedge y \leq x,y$, and therefore $\neg\neg (x\wedge y) \leq \neg \neg x \wedge \neg \neg y$.
     We can prove $\neg\neg x \wedge \neg\neg y \leq \neg\neg (x \wedge y)$ using the definition of
     negation to get $\neg\neg x \wedge \neg\neg y \wedge \neg (x \wedge y) \leq \bot$, and then by reversing
     the definition of implication $\neg\neg y \wedge \neg (x \wedge y) \leq \neg\neg\neg x = x$. Applying
     the same reasoning to $y$, we finally get $x \wedge y \wedge \neg (x \wedge y) \leq \bot$.
  5) Follows from $x \wedge 1 \leq x$.
  6) Using the evaluation morphism, we know that $x \wedge (x \impl y) \leq y \leq x \wedge y$.
...
#+end_proposition
#+begin_proof
Let $a$ a complement of $x$. By definition, $x \wedge a = \bot$ and therefore $a \leq \neg x$.
...
** TODO Lawvere algebraic theories
** Topoi
...
# Selinger Combinatorial algebras
...
** TODO Quotes
# This quotes can be placed on the partial abstracts or in the general one.
