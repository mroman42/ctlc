...
$r \in {\cal C}$ such that there exists an universal arrow $u \colon F \todot \Delta r$ from $F$ to $\Delta$.
...
** Usage
*** Installation
The following script installs the complete Mikrokosmos interpreter, Jupyter kernel
and libraries on a fresh system. It has been tested under =Ubuntu 16.04.3 LTS (Xenial Xerus)=.
...
*** CodeMirror lexer
A simple CodeMirror lexer has been written. It uses Javascript regular expressions
and signals the ocurrence of certain kind operators to CodeMirror.
