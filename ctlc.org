 ...
 * ${\cal C}_0$ (sometimes denoted $\mathrm{obj}({\cal C})$ or simply ${\cal C}$), a collection whose elements are called
...
*** (\infty,1)-categories
There is no consensus on the definition of a *(\infty,1)-category* yet,
but whenever we talk of (\infty,1)-categories, we expect them to have
...
Let $\Delta$ be the category of finite ordinals, with objects of the
form
\[
[n] = (0 < \dots < n),
\]
for any $n \in \mathbb{N}$, and order-preserving maps between them. The category
of *simplicial sets* is the functor category $\Sets^{\Delta^{op}}$.
