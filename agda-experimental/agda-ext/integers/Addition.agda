...
module integers.Addition where
  ...
  +pos : ∀ n m → pos n + pos m ≡ pos (succⁿ (n +ⁿ m))
  +pos zeroⁿ m = refl
  +pos (succⁿ n) m = ap succ (+pos n m)
  {-# REWRITE +pos #-}

  +neg : ∀ n m → neg n + neg m ≡ neg (succⁿ (n +ⁿ m))
  +neg zeroⁿ m = refl
  +neg (succⁿ n) m = ap pred (+neg n m)
  {-# REWRITE +neg #-}  
  ...
  +npos : ∀ n m → pos (n +ⁿ m) ≡ neg 0 + pos n + pos m
  +npos n m = refl
