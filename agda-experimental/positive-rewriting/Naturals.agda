...
open import Base renaming (_+ⁿ_ to _+_) public
open import Booleans renaming (_≡?_ to _≡?ᵇ_ ; from≡? to from≡?ᵇ ; to≡? to to≡?ᵇ) public
...
-- Rewriting on the natural numbers
plus0 : (n : ℕ) → (n + 0) ≡ n
plus0 zero = refl
plus0 (succ n) = ap succ (plus0 n) 
{-# REWRITE plus0 #-}

times0 : (n : ℕ) → n * 0 ≡ 0
times0 zero = refl
times0 (succ n) = times0 n
{-# REWRITE times0 #-}

*assoc : ∀ a b c → (a * (b * c) ≡? a * b * c) ≡ true
*assoc a b zero = refl
*assoc zero b (succ c) = refl
*assoc (succ a) b (succ c) = {!!}



-- cancelfactor2 : ∀ a b c d e → (a * ((succ b) * c)) ≡? (d * ((succ b) * e)) ≡ (a * c) ≡? (d * e)
-- cancelfactor2 a b c d e = 
--   begin
--     (a * ((succ b) * c)) ≡? (d * ((succ b) * e)) ≡⟨ ap (λ u → u ≡? d * (succ b * e)) {!*assoc!} ⟩
--     ((a * (succ b)) * c) ≡? (d * ((succ b) * e)) ≡⟨ {!!} ⟩    
--     (a * c) ≡? (d * e)
--   qed

lemmatimes2 : ∀ a b c d → (a * (b + b) ≡? c * (d + d)) ≡ (a * b ≡? c * d)
lemmatimes2 a b c d = 
  begin
    (a * (b + b) ≡? c * (d + d)) ≡⟨ refl ⟩
    (a * (2 * b) ≡? c * (2 * d)) ≡⟨ {!!} ⟩
...
even-form : (n : ℕ) → odd n ≡ false → Σ ℕ (λ m → 2 * m ≡? n ≡ true)
even-form zero p = zero , refl
even-form (succ zero) ()
even-form (succ (succ n)) p with (even-form n p)
even-form (succ (succ n)) p | m' , q = (succ m') , q
