...
module algebra.HigherGroups where

  record HigherGroupStructure {ℓ} (M : Type ℓ) : Type ℓ where
    constructor higher-group-structure
    field
      -- A group is a monoid
      _*_ : M → M → M
      e : M            
  ...
  open HigherGroupStructure {{...}} public

  HigherGroup : ∀{ℓ} → Type (lsuc ℓ)
  HigherGroup {ℓ} = Σ {lsuc ℓ} {ℓ} (Type ℓ) (HigherGroupStructure {_})
